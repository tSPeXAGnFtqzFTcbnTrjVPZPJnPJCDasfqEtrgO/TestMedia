...
public class CopyIndicator extends View implements IndicatorInterface, ViewPager.OnPageChangeListener {
    ...
    public CopyIndicator(Context context) {
        super(context);
    }

    public CopyIndicator(Context context, AttributeSet attrs) {
        super(context, attrs);

        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.MyIndicatorView);

        this.radiusSelected = typedArray.getDimensionPixelSize(R.styleable.MyIndicatorView_indi_color_select, DEFAULT_RADIUS_SELECTED);

        this.radiusUnselected = typedArray.getDimensionPixelSize(R.styleable.MyIndicatorView_indi_radius_unselect, DEFAULT_RADIUS_UNSELECTED);

        this.distance = typedArray.getInt(R.styleable.MyIndicatorView_indi_distance, DEFAULT_DISTANCE);

        this.colorSelected = typedArray.getColor(R.styleable.MyIndicatorView_indi_color_select, Color.parseColor("#ffffff"));

        this.colorUnselected = typedArray.getColor(R.styleable.MyIndicatorView_indi_color_unselect, Color.parseColor("#ffffff"));

        typedArray.recycle();
    }

    public CopyIndicator(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        for (int i = 0; i < dots.length; i++) {
            ...
            dots[i].setCurRadius(i == currentPosition ? radiusSelected : radiusUnselected);
    ...
    @Override
    public void setViewPager(ViewPager viewPager) throws PageException {
    ...
    private void changeNewRadius(int positionPerform, int newRadius) {
        if (dots[positionPerform].getCurRadius() != newRadius) {
            dots[positionPerform].setCurRadius(newRadius);
